...
from src.schemas.google import Tile, RelatedEvent, TileResponse
from src.utils.api_response import success_response, error_response

# Configure templates and router
templates = Jinja2Templates(directory="templates")
router = APIRouter(prefix="/api/google-tiles", tags=["google-tiles"])
...
def fetch_and_transform_emails(credentials: Dict[str, Any], limit: int = 10) -> List[Any]:
    ...
    if not gmail_adapter:
        ...
        raise HTTPException(
            ...
            detail="Failed to initialize Gmail adapter"
        )
...
async def view_google_tiles(
    request: Request, 
    limit: int = Query(10, description="Maximum number of emails/tiles to fetch")
):
    ...
    try:
        ...
        return JSONResponse(
            content=error_response(
                message=e.detail,
                code="auth_error" if e.status_code == status.HTTP_401_UNAUTHORIZED else "request_error",
        ...
        return JSONResponse(
            content=error_response(
                message=f"Failed to load tiles: {str(e)}",
                code="server_error",
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR
            ),
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR
